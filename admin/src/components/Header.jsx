...
<button onClick={onLogOut} className="hidden md:flex items-center gap-2 bg-red-500 hover:bg-red-600 transition px-4 py-2 rounded-lg shadow">
<LogOut size={18}/>
...
{menuOpen? <X size={24}/> : <Menu size={24}/>}
        ...
        {menuOpen && (
            ...
            {links.map((link) => (
                <Link key={link.path} to={link.path} onClick={()=> setMenuOpen(false)} className={`flex items-center gap-3 px-3 py-3 rounded-lg transition ${location.pathname === link.path ? "bg-white/20 shadow-md" : "hover:bg-white/10"}`}>
                    {link.icon}
                    <span className="text-sm font-medium">{link.name}</span>
                </Link>
            ))}

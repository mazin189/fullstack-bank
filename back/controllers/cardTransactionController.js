...
export const depositToCard = async (req, res) => {
  ...
  try {
    ...
    if (!card)
      return res.status(400).json({ message: "لم يتم العثور علي بطاقة" });
...
export const withdrawFromCard = async (req, res) => {
  ...
  try {
    ...
    if (card.balance < amount)
      return res.status(400).json({ message: "رصيد البطاقة غير كاف" });
    card.balance -= Number(amount);
    await card.save();
...
export const transferToCard = async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "تم تحويل المبلغ إلي البطاقة بنجاح",
...
export const transferToAccount = async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "تم تحويل المبلغ إلي الحساب الرئيسي",
      userBalance: user.balance,
      cardBalance: card.balance,
    });
  } catch (err) {
